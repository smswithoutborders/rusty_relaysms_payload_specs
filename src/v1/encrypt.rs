...
fn v1_token_derivation(
    ...
    hk.expand(info.as_slice(), &mut aes_key).expect("expansion should be ok");

    let nonce = Nonce::try_from([0x00u8; 12]).unwrap();
    let associated_data = [key_id, ec_pk_kid.as_bytes(), es_pk_kid.as_bytes()].concat();
    let payload = Payload {
