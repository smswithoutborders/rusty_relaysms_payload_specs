...
fn v1_token_decrypt_server(
    ...
    let es_kid: [u8; 32] = es_kid.try_into().expect("ss_kid should be 32 bytes");
...
fn test_token_encryption_decryption() {
    ...
    let decrypted = v1_token_decrypt_client(
    ...
    let ciphertext = v1_token_encrypt_client(
        ec_kid.to_bytes().to_vec(),
        ss_kid_pk,
        es_kid_pk,
        key_id,
    ...
    let decrypted = v1_token_decrypt_server(
        ss_kid.to_bytes().to_vec(),
        es_kid.to_bytes().to_vec(),
        ec_kid_pk,
        key_id,
